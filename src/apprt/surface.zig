...
pub const Message = union(enum) {
    ...
    /// [Spike/automation-testing] Inject a synthetic key event into this
    /// surface, as if it came from the user's keyboard. This is produced
    /// by the `GHOSTTY_SENDKEYS_FILE` watcher (see Surface.zig) so it must
    /// stay a plain value (no borrowed slices) since it crosses threads
    /// through the mailbox.
