...
/// [Spike/automation-testing] Background thread that tails a file for
/// synthetic key events and injects them into this surface as if they
/// were typed. Only started if GHOSTTY_SENDKEYS_FILE is set. See
...
pub fn init(
    ...
    // [Spike/automation-testing] Start the sendkeys file watcher if
    // configured. This is not fatal if it fails to start.
    self.sendkeysWatcherStart() catch |err| log.warn(
        "failed to start sendkeys watcher err={}",
        .{err},
    );
}

pub fn deinit(self: *Surface) void {
    // Stop the sendkeys watcher thread, if any (spike/automation-testing).
    if (self.sendkeys_thread) |t| {
        self.sendkeys_stop.store(true, .seq_cst);
        t.join();
    }
...
// ============================================================================
// [Spike/automation-testing] "sendkeys" spool directory watcher.
//
// If GHOSTTY_SENDKEYS_DIR is set to a directory, a background thread
// polls that directory for files and injects their contents into this
// surface as synthetic key events, for driving Ghostty from external
// test processes. This is a spool queue, not a single tailed file, so
// that it scales to many concurrent producers:
//
//   - Any number of processes can drop files into the directory
//     independently -- there's no shared offset/cursor to contend over
//     like there would be with N writers appending to one file.
//   - Each file is processed once, in sorted-filename order, then
//     deleted. Name files with a sortable prefix (e.g. a zero-padded
//     counter or a monotonic timestamp) if cross-file ordering matters;
//     unordered bursts are fine otherwise.
//   - Producers MUST publish atomically: write to a temp file *outside*
//     the watched directory (or as a dotfile inside it -- names starting
//     with '.' are ignored) and rename(2) it into place. rename is
//     atomic on POSIX, so the watcher only ever sees whole files, never
//     a partial write in progress.
//   - A drained pass (>=1 file found) immediately rescans instead of
//     sleeping, so a burst is processed back-to-back; the thread only
//     sleeps once it finds the directory empty. This keeps throughput
//     high under load while staying idle-cheap otherwise.
//   - Delivery is at-most-once: a file is unlinked before its lines are
//     dispatched, so a crash mid-file can't replay it forever. For input
//     injection, dropping a stuck event is preferable to double-typing.
//
// Each file's content is newline-delimited, same two line formats as
// before:
//
//   TEXT:hello world      types the literal text, one key event per rune
//   KEY:ctrl+c            presses+releases a single trigger, using the
//                         same syntax as a config `keybind` trigger
//
// A bare line with no prefix is treated as TEXT.
//
// This never touches the surface directly from the watcher thread --
// events are marshaled onto the main thread via the existing surface
// mailbox (see .inject_key in apprt.surface.Message), the same path
// used by every other cross-thread notification in this file.
// ============================================================================

/// Starts the sendkeys watcher thread if GHOSTTY_SENDKEYS_DIR is set.
/// Does nothing if the env var isn't set. Not fatal on error.
fn sendkeysWatcherStart(self: *Surface) !void {
    const env_result = (try internal_os.getenv(
        self.alloc,
        "GHOSTTY_SENDKEYS_DIR",
    )) orelse return;
    defer env_result.deinit(self.alloc);

    // Always dupe (even on POSIX where the env value is a stable process
    // pointer) since this needs to be an owned buffer the watcher thread
    // can free when it exits.
    const dir_path = try self.alloc.dupe(u8, env_result.value);
    errdefer self.alloc.free(dir_path);

    // Fail fast (and loudly) if the directory doesn't exist yet rather
    // than silently polling a black hole forever. We don't create it
    // ourselves since a spool dir's lifecycle is the automation
    // harness's to own.
    {
...
fn sendkeysWatcherMain(self: *Surface, dir_path: []const u8) void {
    ...
    while (!self.sendkeys_stop.load(.seq_cst)) {
        const drained = self.sendkeysWatcherDrain(dir_path) catch |err| drained: {
            log.warn("sendkeys watcher drain error err={}", .{err});
            break :drained false;
        };

        // Only sleep once a pass finds nothing -- a burst of queued
        // files gets processed back-to-back with no artificial delay.
        if (!drained) std.Thread.sleep(25 * std.time.ns_per_ms);
    }
}

/// Scans the directory once for regular, non-dotfile entries, processes
/// them in sorted filename order (each read-then-delete-then-dispatch),
/// and returns whether at least one file was found.
fn sendkeysWatcherDrain(self: *Surface, dir_path: []const u8) !bool {
    ...
    while (try it.next()) |entry| {
        ...
        // Dotfiles are reserved for producers staging an in-progress
        // write before they rename(2) it into place atomically.
        if (entry.name.len == 0 or entry.name[0] == '.') continue;
        try names.append(self.alloc, try self.alloc.dupe(u8, entry.name));
    }
    ...
    for (names.items) |name| {
        self.sendkeysWatcherProcessFile(dir, name) catch |err| log.warn(
            "sendkeys watcher failed to process file={s} err={}",
            .{ name, err },
        );
    }
...
/// Reads one spool file, deletes it, then dispatches its lines. Deleting
/// before dispatch (rather than after) is deliberate: see the
/// at-most-once note in the header comment above.
fn sendkeysWatcherProcessFile(
    self: *Surface,
    dir: std.fs.Dir,
    name: []const u8,
) !void {
    const data = dir.readFileAlloc(self.alloc, name, 1024 * 1024) catch |err| switch (err) {
        // Raced with another consumer of this same directory, or the
        // producer is still mid-rename. Either way, skip it this pass.
        error.FileNotFound => return,
        else => return err,
    };
    defer self.alloc.free(data);

    dir.deleteFile(name) catch |err| log.warn(
        "sendkeys watcher failed to remove processed file={s} err={}",
        .{ name, err },
    );
    ...
    while (line_it.next()) |raw_line| {
        ...
        self.sendkeysProcessLine(line) catch |err| log.warn(
            "sendkeys watcher failed to process line={s} err={}",
            .{ line, err },
        );
    }
}
...
/// and injects a press+release for it.
fn sendkeysInjectTrigger(self: *Surface, spec: []const u8) !void {
    const trigger = input.Binding.Trigger.parse(spec) catch |err| {
        log.warn("sendkeys watcher invalid KEY spec={s} err={}", .{ spec, err });
        return;
    };
    const key: input.Key = switch (trigger.key) {
        .physical => |k| k,
        .unicode, .catch_all => {
            log.warn("sendkeys watcher unsupported KEY spec={s}", .{spec});
            return;
        },
    };

    self.sendkeysInject(.press, key, trigger.mods, "");
    self.sendkeysInject(.release, key, trigger.mods, "");
}

/// Types literal text, one synthetic key event per Unicode codepoint.
fn sendkeysInjectText(self: *Surface, text: []const u8) !void {
    const view = std.unicode.Utf8View.init(text) catch {
        log.warn("sendkeys watcher invalid utf8 in TEXT line", .{});
        return;
    };
    ...
    while (it.nextCodepoint()) |cp| {
        ...
        const utf8 = cp_buf[0..cp_len];

        const decoded: struct { key: input.Key, utf8: []const u8 } = switch (cp) {
            '\n', '\r' => .{ .key = .enter, .utf8 = "\r" },
            '\t' => .{ .key = .tab, .utf8 = "\t" },
            else => blk: {
                const lower: u21 = if (cp < 128) std.ascii.toLower(@intCast(cp)) else cp;
                const k: input.Key = if (lower < 128)
                    (input.Key.fromASCII(@intCast(lower)) orelse .unidentified)
                else
                    .unidentified;
                break :blk .{ .key = k, .utf8 = utf8 };
            },
        };

        self.sendkeysInject(.press, decoded.key, .{}, decoded.utf8);
        self.sendkeysInject(.release, decoded.key, .{}, "");
    }
}

/// Marshals a synthetic key event onto the main thread via the surface
/// mailbox, where it's processed like any other apprt-originated key
/// event (see .inject_key in Surface.handleMessage).
fn sendkeysInject(
    self: *Surface,
    action: input.Action,
    key: input.Key,
    ...
    var msg: apprt.surface.Message.InjectKey = .{
        .action = action,
        .key = key,
        .mods = mods,
    };
    msg.utf8 = apprt.surface.Message.WriteReq.init(
        self.alloc,
        @as([]const u8, utf8),
    ) catch return;
